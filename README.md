...
面向 **Lua 5.1.5** 与 **Luau 0.735 / Roblox 方向** 的 std-only Rust 工具链。默认 `virtualize` 已实现真正的 **AST → IR → 自定义 Bytecode → 寄存器 VM**：公开 `.obf` 固定 **32-byte Header**，指令操作数以 **7-bit varint** 序列化（小数值 1 byte，较大值 2~N bytes），并有独立常量/捕获/prototype。生成器不依赖原生 compiler，不用 `load/loadstring` 委托执行；原生后端只通过 `--backend native` 显式选择。`.obf` 文件保持明文 canonical ISA2、与 seed 无关；包装成脚本时先降为私有 seed-specific **ISA5 semantic image**：直线指令组合为 1~4 primitive recipe superoperator，use-site 不带 opcode、稳定 recipe ID 或明文 successor，而是携带三阶段 edge token 与绑定动态后继的五阶段 recipe token；每次 fetch 都经过两套乱序状态机及多层不透明谓词/高密度死状态。每个真实 prototype 的入口和抽样 CFG 边还会插入实际可达、实际执行的中性 bundle，随后与原 record 一起物理洗牌；兄弟 prototype 重排且混入 2~4 个不可达 synthetic prototype 子树节点。随后才叠加 M6 双层传输混淆、base86 分段和最终一/两字母改名。该设计针对“解码后逐 opcode 一对一翻译”的静态恢复路径，但仍是可逆混淆，不宣称不可破解。
...
source → 现有 AST / BindingId → typed register IR / basic blocks
       ...
       → seed semantic lowering（recipe + edge/recipe token + reachable neutral graph）→ private ISA5
       → 目标端校验 / recipe register VM → 最终随机短名 / 单行化
```

这条链路**不调用原生 compiler，不存 native word，也不默默 fallback**。新 `ir::Module` 包含函数、常量、cell/upvalue 捕获和带符号后继的基本块；IR 的 branch 生成 `Test + Jump + Jump`。公开文件 Header 固定 **32 bytes**，含版本、目标、端序、宽度码、文件长度、prototype 数量、入口、ISA 版本和 Adler-32；canonical 指令流按 Form 写成 `[opcode][各字段 varint]`（A/AB/ABC/ABx/Ax，2~7 bytes）。`compile`/`inspect-bytecode`/`serialize` 仍遵守该公开 ISA2 规范；只有 `virtualize`/`wrap-bytecode` 在生成脚本内部把已验证 Program 重编码为 ISA5 recipe/edge-token graph。完整逐字段规范与 **49 条 primitive ISA** 见 [`自定义字节码.md`](自定义字节码.md)。
...
所有 decoder、runtime、dispatcher、handler、静态方法适配器和执行尾部组装完后，自定义 VM 先缩短私有字段，再调用一次私有 `minify::finalize_vm` 统一随机 local、分号分隔和单行化；之后不追加代码。重解析复核绑定图、同域唯一性、名称长度和确实换名；双层解密后的内容须等于同一 Program/seed 确定性重编码出的 ISA5 semantic image，而不再等于输入 canonical `.obf`。生成器环境例外仍严格审计：`local G=(getfenv and getfenv(0))or _G`，加上三个 payload 密钥函数内**形状完全固定**的环境探针（Luau `debug and debug.info(loadstring,"s")` 结果须为 `"[C]"`；Lua 5.1 `debug and debug.getinfo(loadstring,"S")` 须 `what=="C"`）。探针不通过则静默中止、无任何输出；没有公开忽略反射的开关。

### 私有 semantic image 与 payload 传输层

- **ISA5 语义虚拟化（针对静态破解报告，第二阶段）**：`generate` 不再嵌入调用者传入的 canonical 指令 bytes，而是从已验证 `Program` 确定性构造 seed-specific image。直线区随机组合为 1~4 primitive recipe；每个真实 prototype 的入口保证插入 2~4 个 neutral bundle，并按函数规模从真实 next/test edge 抽取至多 5 条、各插入 1 个。正常 frame 使用新增私有 scratch register 执行 `Nil/Not/Move/Clear` 后恢复中性，256-register 满帧安全退化为 `Move r0,r0`；这些节点有真实入边且运行时会被 fetch。record 的两个后继槽改存三层 edge token，每层绑定 source label、prototype、edge kind 与交叉项；验证及每次 fetch 通过独立 ED 状态机动态恢复后继，再把后继作为五层 RD recipe token 的上下文。ED 含 3 live/3 dead state 与每层 2~3 重谓词；RD 含 5 live/5 dead state 与每层 4~5 重谓词。持久 code record 不缓存明文后继。record/兄弟 prototype 仍洗牌，Closure Bx 同步重排，并混入 2~4 个 live-tree 不可达 synthetic prototype。四个 graph-unreferenced poison recipe 仍保留，和新增 graph-referenced/executed neutral recipe 分属两类。
- 生成时用 seed 派生的 **Lehmer 密钥流（48271 mod 2147483647）** 对上述 ISA5 image 逐字节 XOR，密文字节分布均匀（Shannon 熵 > 7.5 bits/byte，测试断言高于明文）；所有中间量 < 2^53，Lua 双精度与 Rust 逐位一致。
...
- **M7 结构随机化（不含外层包装变体）**：seed 驱动的等价结构变体进入默认路径——F3 primitive 校验按 `o`、ISA5 superhandler 按动态恢复出的完整 u16 `rid` 分别拆为 2~4 个 grouped dispatch；每条比较随机取 `x==K`/`K==x`/`not(x~=K)`/`x-K==0`，整数边界与元方法分支也有独立等价变体。`tools/bench-vm.sh` 继续看护性能；semantic superhandler 有意识用体积换去同构，当前硬预算为 **72,000/80,000 B**（Lua51/Luau），单次运行灾难阈值仍为 1500ms。
- **不透明真假分支与两类 semantic decoy**：入口方法体仍以恒真/恒假式包住实际链和不运行的真指令，F3 还保留不可能 primitive 值的死臂；ISA5 同时保留 dictionary-referenced-only 的 poison recipe、两套 token decoder dead state，并新增 graph-referenced/executed neutral bundle。前者只能通过 reachability 排除且声明/执行序列故意不一致；后者不能靠“无 record 引用”删除，必须进一步做中性效果分析。
- 入口按 **seed 洗牌的顺序**调用三个函数并传入各自的结构键对，decoder 段以 `1+(s1+s2+s3+31*#B)%2147483646`（混入密文长度）重建密钥流并继续 magic/Adler/ISA5 edge/recipe-token/图结构校验；份额与密钥流初态的十进制串不出现在脚本任何位置（单元测试逐 seed 断言）。`.obf` 文件与 `compile` 输出保持明文 canonical、与 seed 无关；`vm::custom::decrypt_embedded` 现在返回解开两层后的**私有 ISA5 image**，用于和同 Program/seed 的 deterministic semantic re-encode 对照。
...
|---|---|---:|---:|---:|
| `vm_lua51.out.lua` | `tests/fixtures/vm_lua51.lua` | 7001 | 5,525 B | 71,084 B |
| `vm_luau.out.lua` | `tests/fixtures/vm_luau.lua` | 7351 | 6,575 B | 78,081 B |
...
4. 对 ISA5 fetch 在 ED/RD 动态解出 edge/recipe token 后做运行插桩，再展开实际 recipe 的 primitive 序列确认覆盖 **Lua51 46/46、Luau 49/49**；同时证明入口 neutral recipe 实际执行、未引用 poison recipe 不执行；
...
13. `tests/private_fields.rs` 与私有字段内部测试检查 14 字段双射、单/双字母池、关键字/冲突/越界拒绝、marker-like 用户方法名、公开字段/导出保护、CLI/修订 1 包装，以及输入 `.obf` 仍 canonical、内嵌内容则严格为 ISA5 semantic image。
...
默认 AST/IR/v2 register VM、独立 reader/encoder、完整 primitive ISA、私有 ISA5 semantic/edge-token lowering、可达 neutral CFG split 与最终随机短名均已可运行。后续优先扩展真实业务语料、语义/压力差分、寄存器/pack 开销及宿主边界，并继续实现 semantic fusion/splitting、动态密钥/分组加密、全局 prototype/function segment pool 及函数 inlining/outlining/reorder；不能把现有 recipe/graph 层描述成不可逆，也不应再以单纯增加传输加密替代反虚拟化结构改进。
