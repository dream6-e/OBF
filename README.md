...
source → 现有 AST / BindingId → typed register IR / basic blocks
       ...
       → private ISA12（per-prototype operand/register ABI + global fragments/segment graph）
...
- Lua 5.1：46 个 `src/vm/opcode/lua51/c*.rs`；Luau：49 个 `src/vm/opcode/luau/c*.rs`。它们现在是 primitive 语义模板；生成器先以 lexer token span 把模板中的每个 `R[index]` 降为当前 frame mapper 的物理 key（字符串/注释不参与），再把程序的 1~4-op recipe 拆成 1~2-op fragments，经随机 `sid` continuation 执行；不在 use-site 上做固定 opcode→handler 或 recipe→连续 body 分派，也不为每个 prototype 复制整套 handler。
...
- **ISA12 私有像**：语义 schema 继承 ISA11 的 validation-equivalent descriptor、1~2 primitive global fragments、三层 edge token、五层 recipe token、reachable neutral bundle、synthetic subtree、record/prototype 洗牌及 cross-prototype two-node code-segment graph；ISA12-A 新增不自文档化为 profile 表的 per-prototype operand ABI，ISA12-B 再为 frame/handler 加入无完整映射表的 per-prototype register ABI。header revision 保持 12，旧 decoder 不会静默接收新产物。
...
当前 ISA12-B 完整矩阵为 **PASS188（74 单元 + 114 集成）**；Lua51/Luau 原生语法、运行、debug/release、golden、ChaCha8/anti-hook/corruption、operand/register ABI 与 **semantic-bytecode→完整LZW-frame strict-smaller** 门全部通过。默认15-run `tools/bench-vm.sh` 最近结果为Lua51 **96 ms / 48.0x**、Luau **115 ms / 38.3x**。golden整脚本为74,540/83,040 B；它们与历史ISA的大小关系仅作信息展示，不是压缩验收条件。
...
默认 AST/IR/v2 register VM、完整 primitive ISA、private ISA12 semantic graph、per-prototype operand ABI与register ABI/handler register-access lowering、runtime-witness/data-dependent control、bounded LZW、双 ChaCha8、每次解密 anti-hook、frame v2 与最终随机短名均已可运行。破解报告暴露的统一 `6+3*i` operand slot、canonical actual-op marker及全像统一逻辑 `R[index]` 物理键已切断；但 mapper公式和 primitive 运算结构仍随客户端交付，计算profile后仍可逆。下一批应实施真正跨 primitive dataflow fusion，并继续 parser/入口/recipe/CFG 去自文档化；captures/constants 全局池及安全 inlining/outlining/reorder仍须独立评估。继续堆叠 encryption/encoding/compression 不能替代这些结构工作，也不得宣称客户端秘密、不可逆或“静态恢复已解决”。
