...
面向 **Lua 5.1.5** 与 **Luau 0.735 / Roblox 方向** 的 std-only Rust 工具链。默认 `virtualize` 实现 **AST → IR → 自定义 Bytecode → 寄存器 VM**：公开 `.obf` 固定 32-byte Header 与 canonical ISA2/7-bit varint，保持明文且与 seed 无关；生成脚本则降为 seed-specific **private ISA12**。ISA12-A 在既有 validation-equivalent descriptor、edge/recipe token、全局 semantic fragment 与跨 prototype code-segment graph 上增加每 prototype 异构 operand ABI：4 种物理 record family、3 种分量旋转与 2,731 个稀疏 lane 组成 32,772 个无碰撞 profile，覆盖 private image 的 32,767 prototype 上限；fragment 使用四种轮换且重洗牌的返回绑定，不再发出 canonical `6+3*i` operand bridge 或实际 opcode 数字标记。ISA12-B 又以模 `4×127×257=130,556` 的 seed-derived family/stride/shift profile，把每个 prototype 的逻辑寄存器 `0..255` 单射到四个互斥 257-key bank 之一；profile 每 frame 只推导一次且不输出 256 项映射表，`SETUP` 与全部 primitive handler 的 `R[index]` 都经同一 token-level mapper 访问。ISA12-C 进一步只为可安全分析的相邻 primitive 生成真正的 carried-value fusion：首操作的 `R[a]` 结果只求值一次并保留逻辑目标，次操作的每个 register read 经当前 frame family 的 `RF(index,target,value)` 转发，命中依赖时直接使用 carry（包括 `nil/false`），不命中才走 `RX`；不安全、多写及控制操作保持单 fragment，不再把两个原始 handler 文本相邻拼接冒充 fusion。入口的稳定线性接线也被限定为六状态 seed-shuffled stage graph。随后仍按 `bounded LZW → inner ChaCha8 → strict frame v2 → outer ChaCha8 → base86 三段` 交付。ChaCha8 使用标准 IETF 256-bit key/32-bit counter/96-bit nonce 布局与 8 rounds；目标端纯算术实现不依赖 `bit`/`bit32`，并拆成 32-bit XOR/rotate、quarter-round、block、KDF/stream、anti-hook 五个随机数字键 sibling fields，随全部 payload fields 全局洗牌。最终 key/nonce/counter 由三个运行时 source-witness shares、重建的 `pv`、domain/context 和 anti-hook attestation 经 ChaCha8 block 动态派生，不作为脚本字面量保存；外/内两次解密前都执行 anti-hook，检查关键 native source、generated helper 同源、基础 primitive 行为和 ChaCha8 known-answer test，任一异常在 frame/LZW/semantic parser 与用户代码前 fail closed。该设计仍是可逆混淆：ciphertext、decoder、salt、register/dataflow mapper 与规范环境都随客户端交付，持有完整脚本者仍可执行或精确模拟恢复；ISA12-C 已削弱逐 primitive 原始 handler 边界和顶层稳定入口链，但 fusion 条件、`RF`、primitive 运算、parser inverse 与依赖图仍在客户端，不能宣称已解决全部静态语义恢复。
...
source → 现有 AST / BindingId → typed register IR / basic blocks
       ...
       → private ISA12-C（operand/register ABI + carried-value fragments + segment graph）
...
这条链路**不调用原生 compiler，不存 native word，也不默默 fallback**。新 `ir::Module` 包含函数、常量、cell/upvalue 捕获和带符号后继的基本块；IR 的 branch 生成 `Test + Jump + Jump`。公开文件 Header 固定 **32 bytes**，含版本、目标、端序、宽度码、文件长度、prototype 数量、入口、ISA 版本和 Adler-32；canonical 指令流按 Form 写成 `[opcode][各字段 varint]`（A/AB/ABC/ABx/Ax，2~7 bytes）。`compile`/`inspect-bytecode`/`serialize` 仍遵守该公开 ISA2 规范；只有 `virtualize`/`wrap-bytecode` 在生成脚本内部把已验证 Program 重编码为 ISA12 camouflaged-recipe/edge-token/segment graph，并在目标 parser、frame setup 与 fragment 之间使用 per-prototype operand/register ABI，严格无损压缩后再进入双 ChaCha8 与 frame v2 transport。完整逐字段规范与 **49 条 primitive ISA** 见 [`自定义字节码.md`](自定义字节码.md)。
...
- **ISA12 私有像 / ISA12-C 生成结构**：wire schema 继承 ISA11 的 validation-equivalent descriptor、三层 edge token、五层 recipe token、reachable neutral bundle、synthetic subtree、record/prototype 洗牌及 cross-prototype two-node code-segment graph；ISA12-A 加入 per-prototype operand ABI，ISA12-B 加入无完整映射表的 register ABI。ISA12-C 不改 semantic bytes/header revision，而是在 handler emission 上把安全相邻项改为 carried-value 2-op fragment，并把 entry wiring 改成六状态图；所以 private header revision 仍为 12。
...
|---|---|---:|---:|---:|
| `vm_lua51.out.lua` | `tests/fixtures/vm_lua51.lua` | 7001 | 5,525 B | 74,540 B |
| `vm_luau.out.lua` | `tests/fixtures/vm_luau.lua` | 7351 | 6,575 B | 83,040 B |

SHA-256：Lua51 `927b84824299ab9a45df3a5aad3db67d016ed11a7ab31bc2155ea1029e4e7dd1`；Luau `135514905e4e101cc282b0d58b098426fb2dc2bee5a9e4424bb46113685f758c`。
...
4. 对 ISA12 fetch 在 ED/RD 动态解出 edge/recipe token 后做运行插桩，再按独立 `sid` fragment graph 展开真实 primitive 序列确认覆盖 **Lua51 46/46、Luau 49/49**；同时证明入口 neutral recipe 实际执行、未引用 poison recipe 不执行、live descriptor 谎报不弱于 45%，并断言每个 2-op fragment 恰有一次 producer carry、至少一次 consumer `RF` read，完整 fixture 的 live fusion 数量超过 poison 上限；
...
当前 ISA12-C 完整矩阵为 **PASS190（75 单元 + 115 集成）**；Lua51/Luau 原生语法、运行、debug/release、golden、ChaCha8/anti-hook/corruption、operand/register ABI、dataflow fusion、entry graph 与 **semantic-bytecode→完整LZW-frame strict-smaller** 门全部通过。默认15-run `tools/bench-vm.sh` 最近结果为Lua51 **96 ms / 48.0x**、Luau **117 ms / 39.0x**。golden整脚本为78,706/87,167 B；它们与历史ISA的大小关系仅作信息展示，不是压缩验收条件。
...
默认 AST/IR/v2 register VM、完整 primitive ISA、private ISA12 semantic graph、per-prototype operand/register ABI、ISA12-C carried-value fusion、六状态 entry graph、runtime-witness/data-dependent control、bounded LZW、双 ChaCha8、每次解密 anti-hook、frame v2 与最终随机短名均已可运行。破解报告暴露的统一 `6+3*i` operand slot、canonical actual-op marker、全像统一逻辑 `R[index]` 物理键、逐 primitive 原始 handler 拼接边界及顶层稳定线性入口链均已被针对性削弱；但 profile/fusion规则、`RF`、primitive算术/表操作、parser字段及全部 inverse 仍随客户端交付，计算或模拟后仍可逆。下一批应继续降低 semantic parser/recipe/CFG 字段顺序的自文档化；captures/constants 全局池及安全 inlining/outlining/reorder仍须独立评估。继续堆叠 encryption/encoding/compression 不能替代这些结构工作，也不得宣称客户端秘密、不可逆或“静态恢复已解决”。
