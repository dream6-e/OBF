...
- Rust reader 先完整验证 canonical `.obf`；semantic lowering 再按 CFG leader 切分（Jump/Test/terminal 不跨 bundle），给 recipe 分配随机非零 u16 ID，把 Jump 目标改为随机 label，并写入显式 `next/skip`。目标端二次验证 recipe dictionary、canonical operand varint、label 唯一性、全部后继与 prototype/Closure 关系，再以 `code[label]` 图 fetch 执行；不存在可按 `pc += 4` 排序的内嵌原始指令串。
