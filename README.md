...
- **有界无损 LZW**：semantic image 每 8,192-byte 输出重置 dictionary，并保存连续 canonical LSB-first bitstream。16-byte header 为 `LZW\1 | original_len:u32 | bit_len:u32 | Adler32:u32`；完整 frame 不严格小于原像时生成失败。按golden seed的正确比较边界：Lua51 `12,332→11,329 B`（含header，减少1,003 B），Luau `15,559→15,104 B`（含header，减少455 B）。目标端限制原长≤16 MiB、reset≤2,048，并拒绝截断、越界/forward code、非canonical literal、输出越界、非零padding、未完整消费和checksum不符。
...
|---|---|---:|---:|---:|
| `vm_lua51.out.lua` | `tests/fixtures/vm_lua51.lua` | 7001 | 5,525 B | 93,044 B |
| `vm_luau.out.lua` | `tests/fixtures/vm_luau.lua` | 7351 | 6,575 B | 102,734 B |

SHA-256：Lua51 `7cf2a36d9e21dc4233f8f210fb3e0c82af501cef7ba574436792871fa42ef64e`；Luau `8717cd1784887a14873a4643770957c41ffcd8ab1f2e4b79b7766db857372d32`。
...
当前 ISA15 完整矩阵为 **PASS258（142 单元 + 116 集成）**；Lua51/Luau 原生语法、运行、debug/release、golden、ChaCha8/anti-hook/corruption、operand/register/field-order ABI、capture/constant pools、dataflow fusion、entry graph、混合 base86 段传输与 **semantic-bytecode→完整LZW-frame strict-smaller** 门全部通过。默认15-run `tools/bench-vm.sh` 最近结果为Lua51 **123 ms / 61.5x**、Luau **136 ms / 45.3x**。golden整脚本为93,044/102,734 B；它们与历史ISA的大小关系仅作信息展示，不是压缩验收条件。（K系列第1批 K9a 已落地，见`总路线.md` §5。）
