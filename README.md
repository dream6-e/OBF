...
- **有界无损 LZW**：semantic image 每 8,192-byte 输出重置 dictionary，并保存连续 canonical LSB-first bitstream。16-byte header 为 `LZW\1 | original_len:u32 | bit_len:u32 | Adler32:u32`；完整 frame 不严格小于原像时生成失败。目标端限制原长 ≤16 MiB、reset ≤2,048，并拒绝截断、越界/forward code、非 canonical literal、输出越界、非零 padding、未完整消费和 checksum 不符。
...
生成器、命名或分隔策略变更后必须再生成两份示例。矩阵比较默认生成、独立 compile/wrap、debug/release 及 golden 的逐字节一致性；`tools/bench-vm.sh` 还硬性要求两份 ISA11 golden 分别严格小于 ISA7（压缩前）的 83,640/92,116 B。该比较只证明当前固定语料连同 Lua 解压代码后的净体积下降，不代表任意输入都可压缩；不可压缩 frame 会被生成器拒绝。
...
当前 ISA11 完整矩阵为 **PASS185（72 单元 + 113 集成）**；Lua51/Luau 原生语法、运行、debug/release、golden、ChaCha8/anti-hook/corruption 与 strict-smaller 门全部通过。默认 15-run `tools/bench-vm.sh` 结果：Lua51 **93 ms / 46.5x**，Luau **112 ms / 37.3x**；golden 分别为 82,201/90,866 B，均严格小于 ISA7 的 83,640/92,116 B。
