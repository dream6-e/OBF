...
// The per-opcode routine *data* lives in a sibling module so that `seed.rs`
// stays under the 80 KiB source ceiling; re-exported here so every existing
// `seed::routine_*` and `custom::*` path keeps resolving.
pub(crate) use super::seed_routines::routine_for;
...
pub(crate) fn asm_ins(words: Vec<u32>) -> AsmItem {
    AsmItem::Instr(words)
}

pub(crate) fn asm_lab(name: &str) -> AsmItem {
    AsmItem::Label(name.to_owned())
}

pub(crate) fn asm_jif(cond: u32, label: &str) -> AsmItem {
    AsmItem::Jif {
        cond,
...
pub(crate) fn asm_jmp(label: &str) -> AsmItem {
    AsmItem::Jmp {
        label: label.to_owned(),
    }
}
...
#[cfg(test)]
mod tests {
    use super::super::seed_routines::{routine_jump, routine_return, routine_test};
    use super::*;
