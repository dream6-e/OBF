...
fn erased_types_dead_branches_and_transitive_captures_use_runtime_bindings_only() {
    ...
    // K4 reorders module.functions (the entry is pinned, the rest re-roll),
    // so source-order indices are no longer a contract here. Bind by
    // structure instead: exactly one of the two one-parameter top-level
    // closures -- the factory, whose `parameter` is referenced only in type
    // positions -- has an entirely capture-free closure subtree, while the
    // live chain still binds its parameter for the `+=`.
    let roots: Vec<usize> = (0..module.functions.len())
        .filter(|&i| module.functions[i].parent == Some(0) && module.functions[i].parameters == 1)
        .collect();
    assert_eq!(roots.len(), 2, "factory and live moved");
    let subtree_captures = |root: usize| -> usize {
        let mut stack = vec![root];
        let mut total = 0usize;
        while let Some(i) = stack.pop() {
            total += module.functions[i].captures.len();
            stack.extend(
                (0..module.functions.len()).filter(|&j| module.functions[j].parent == Some(i)),
            );
        }
        total
    };
    assert_eq!(
        roots.iter().filter(|&&r| subtree_captures(r) == 0).count(),
        1,
        "exactly one type-only chain must be capture-free"
    );
