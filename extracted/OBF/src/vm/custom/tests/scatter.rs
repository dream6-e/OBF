...
/// K6 第二步的三种折叠写法（同一组装载点用同一种）：`ck=ck+K` /
/// `ck=K+ck` / `ck=ck+(K)`。三种是同一次加法，checkpoint 期望值不变。
fn fold_tails(key: u64) -> [String; 3] {
    [
        format!("end;ck=ck+{key};"),
        format!("end;ck={key}+ck;"),
        format!("end;ck=ck+({key});"),
    ]
}

/// 找装载点的结尾；返回 `(终点, 折叠写法编号)`。
fn install_span(raw: &str, key: u64, from: usize) -> Option<(usize, usize)> {
    for (form, tail) in fold_tails(key).iter().enumerate() {
        if let Some(at) = raw[from..].find(tail) {
            return Some((from + at + tail.len(), form));
        }
    }
    None
}

/// 扫描原始（未缩短名）文本，返回 `(装载点 (键, 起点, 终点, 折叠写法), 字面量字段键)`。
fn scan_scatter(raw: &str) -> (Vec<(u64, usize, usize, usize)>, Vec<u64>) {
    ...
    while let Some(at) = raw[cursor..].find("[") {
        ...
        if is_install {
            if let Some((end, form)) = install_span(raw, key, start) {
                installs.push((key, start, end, form));
                cursor = end;
                continue;
            }
            // 装载语句必须紧跟三种折叠写法之一，否则它不进折叠检查。
...
fn k20_runtime_installs_are_dispersed_and_cover_the_fold_checkpoint() {
    for (target, fixture, seeds) in [
        ...
        let mut forms_seen = std::collections::BTreeSet::new();
        for seed in seeds {
            let raw = generate(&data, &program, seed).unwrap();
            let (installs, literal) = scan_scatter(&raw);
            for (_, _, _, form) in &installs {
                assert!(*form < 3, "{target} seed {seed}: fold form out of range: {form}");
                forms_seen.insert(*form);
            }
            let installed: BTreeSet<u64> = installs.iter().map(|(key, _, _, _)| *key).collect();
            ...
            for key in &installed {
                ...
                let sites = installs.iter().filter(|(site, _, _, _)| site == key).count();
                assert!(
            ...
            let expected = installs
                .iter()
                .fold(0u64, |acc, (key, _, _, _)| acc.wrapping_add(*key));
            let marker = format!("if ck~={expected} then E()end;");
            assert!(
                raw.contains(&marker),
                "{target} seed {seed}: fold checkpoint missing or stale"
            );
            // 写法 0/2 的折叠语句都含 `;ck=ck+` 子串，写法 1 不含；逐装载点
            // 分类后两种计数必须各自闭合。
            let wide = installs.iter().filter(|(_, _, _, f)| *f != 1).count();
            let tight = installs.len() - wide;
            assert_eq!(
                raw.matches(";ck=ck+").count(),
                wide,
                "{target} seed {seed}: `;ck=ck+` count disagrees with form-0/2 installs"
            );
            assert_eq!(
                raw.matches("+ck;").count(),
                tight,
                "{target} seed {seed}: `+ck;` count disagrees with form-1 installs"
            );
            assert_eq!(expected, super::loader::fold_of(&{
                installs.iter().map(|(key, _, _, _)| *key).collect::<Vec<_>>()
            }));
        ...
        // K6 第二步的分组多态：跨种子至少出现两种折叠写法（每组一种，组数
        // 2..=4 由规划器测试钉住），单一写法意味着分组退化。
        assert!(
            forms_seen.len() >= 2,
            "{target}: fold forms are seed-independent: {forms_seen:?}"
        );
    }
}
...
fn k20_install_plan_is_reproducible_and_topologically_valid() {
    ...
    let second = super::loader::scatter(pool.clone(), &mut crate::random::Prng::lcg(0x51a7_7e11_20c0_ffee));
    assert_eq!(
    ...
    // K6 第二步：组数在 2..=4，折叠写法与装载点一一对应。
    assert!(
        (2..=4).contains(&first.groups),
        "group count {:#} outside the 2..=4 band", first.groups
    );
    assert_eq!(
        first.forms.len(),
        first.plan.len(),
        "one fold form per install site"
    );
    for form in &first.forms {
        assert!(*form < 3, "fold form out of range: {form}");
    }
    // 组数本身要随种子动，否则「分组」是假的。
    let group_counts: Vec<usize> = (0..8u64)
        .map(|salt| {
            let mut prng = crate::random::Prng::lcg(0x51a7_7e11_20c0_ffee ^ salt);
            super::loader::scatter(pool.clone(), &mut prng).groups
        })
        .collect();
    assert!(
        group_counts.iter().collect::<std::collections::BTreeSet<_>>().len() >= 2,
        "group count never moves with the seed: {group_counts:?}"
    );
...
fn k20_tampering_with_an_install_statement_faults_the_program() {
    ...
    let keys: Vec<u64> = installs.iter().map(|(key, _, _, _)| *key).collect();
...
/// K6 第二步：新字面对与槽位号模板的逐点等价证明。
///
/// 每一族都是一个纯代数恒等式；这里按族穷举（值域取双精度精确整数范围，
/// 与壳里 Lua number 的语义一致）。模板的捕获只是 `g[NN]` 的槽位号/下标，
/// 不参与代数，用代表性取值代入即可。
#[test]
fn k20_new_respelling_pairs_are_provably_equivalent() {
    // `p>N-n` 与 `p+n>N`、`total+sn>lim` 与 `sn>lim-total`：两边同加。
    for p in 0..=512i64 {
        for n in 0..=512i64 {
            assert_eq!(p > n - 300, p + 300 > n, "guard move at p={p},n={n}");
        }
    }
    for total in 0..=16384i64 {
        for sn in 0..=8192i64 {
            let lim = 8192;
            assert_eq!(total + sn > lim, sn > lim - total, "limit move at {total},{sn}");
        }
    }
    // `MF((n+m-1)/m)` 与 `1+MF((n-1)/m)`：math.floor 语义下的上取整，
    // 对 n=0 也成立（Lua 的 / 对 2 的幂除数在 2^53 内精确）。
    for (m, nmax) in [(8u64, 64u64), (8192, 8192 * 3 + 7)] {
        for n in 0..=nmax {
            let a = ((n as f64 + (m - 1) as f64) / m as f64).floor();
            let b = 1.0 + ((n as f64 - 1.0) / m as f64).floor();
            assert_eq!(a, b, "ceil form at m={m}, n={n}");
        }
        for n in [0u64, 1, m - 1, m, m + 1, 16_777_216] {
            let a = ((n as f64 + (m - 1) as f64) / m as f64).floor();
            let b = 1.0 + ((n as f64 - 1.0) / m as f64).floor();
            assert_eq!(a, b, "ceil form (large) at m={m}, n={n}");
        }
    }
    // `(4-(k)%4)%4` 与 `(-k)%4`：Lua 的 % 是 floor-mod（结果取除数符号）。
    let lmod = |x: i64, m: i64| ((x % m) + m) % m;
    for n in 0..=64i64 {
        let k = 16 + n;
        assert_eq!(lmod(4 - lmod(k, 4), 4), lmod(-k, 4), "pad form at n={n}");
    }
    // FNV 形累加的模内加数交换：中间量都 < 2^53，双精度精确。
    for left in [0u32, 1, 2048, 65_496] {
        for byte in [0u32, 7, 255] {
            for other in [0u32, 13, 262] {
                let la = f64::from(left) * 257.0 + f64::from(byte);
                let lb = f64::from(byte) + f64::from(left) * 257.0;
                assert_eq!((la % 65_497.0) as u64, (lb % 65_497.0) as u64, "left FNV at {left},{byte}");
                let ra = f64::from(left) * 263.0 + f64::from(byte) + f64::from(other);
                let rb = f64::from(byte) + f64::from(other) + f64::from(left) * 263.0;
                assert_eq!((ra % 65_497.0) as u64, (rb % 65_497.0) as u64, "right FNV at {left},{byte},{other}");
            }
        }
    }
    // 高位拆分：`j-j%256` 与 `j-a`（`a=j%256` 紧邻在前），除法同式。
    for j in [0u64, 1, 255, 256, 257, 65_536, 1_000_003] {
        let a = j % 256;
        assert_eq!(((j - j % 256) / 256) as f64, ((j - a) / 256) as f64, "high split at j={j}");
    }
    // `x+2<L` 与 `x<L-2`：整数同加。
    for x in 0..=300i64 {
        for l in 0..=300i64 {
            assert_eq!(x + 2 < l, x < l - 2, "len guard at x={x},L={l}");
        }
    }
    // 加数交换族（`b=b+1`/`DP[nx]=prev*256+pf`/`TU=TU+nu`/`*87`/`vv=vv+cc*mm`
    // 以及四条槽位号模板）：同一加法项交换，捕获只是下标。
    for x in [0u32, 1, 86, 255] {
        for y in [0u32, 1, 87, 255] {
            assert_eq!(x as u64 + 1, 1 + x as u64, "count swap at x={x}");
            assert_eq!(u64::from(x) * 256 + u64::from(y), u64::from(y) + u64::from(x) * 256, "lzw swap at x={x},y={y}");
            assert_eq!(
                u64::from(x) + u64::from(y) * 87,
                u64::from(y) * 87 + u64::from(x),
                "87 swap at x={x},y={y}"
            );
            assert_eq!(u64::from(x) + u64::from(x) * u64::from(y) * u64::from(y), u64::from(x) * u64::from(y) * u64::from(y) + u64::from(x), "seg swap at x={x},y={y}");
        }
    }
    // varint 模板：`v=v+w%128*128` 与 `v=w%128*128+v`（% 和 * 同优先级左结合）。
    for w in [0u32, 1, 127, 128, 255] {
        let a = w % 128;
        let la = 4096u64 + u64::from(a) * 128;
        let lb = u64::from(a) * 128 + 4096;
        assert_eq!(la, lb, "varint swap at w={w}");
    }
}

/// K6 第二步：新 B 写法必须真的铺进壳里——跨种子、跨目标观察到的不同新
/// B 形态数不得少于 8（字面 15 种 + 槽位号模板 4 种；模板按结构片段计数）。
#[test]
fn k20_new_b_forms_spread_across_seeds() {
    /// 数 `*g[digits]+g[digits]` 形态（三段 base86 累加的 B 写法）。
    fn seg_b_count(raw: &str) -> usize {
        let bytes = raw.as_bytes();
        let mut count = 0usize;
        let mut i = 0usize;
        while let Some(at) = raw[i..].find("*g[") {
            let j = i + at + 3;
            let mut k = j;
            while k < bytes.len() && bytes[k].is_ascii_digit() {
                k += 1;
            }
            if k > j && raw[k..].starts_with("]+g[") {
                count += 1;
            }
            i = j.max(i + 1);
        }
    ...
    let literals = [
        ...
        "p+n>N",
        "DP[nx]=pf+prev*256",
        "DP[nx]=first+prev*256",
        "sn>lim-total",
        "bl=1+MF((bits-1)/8)",
        "cc=1+MF((n-1)/8192)",
        "pad=(-(16+n))%4",
        "left=(byte+left*257)%65497",
    ...
    for (target, fixture, seeds) in [
        ...
        for needle in &literals {
            let found = seeds
                .iter()
                .any(|seed| generate(&data, &program, *seed).unwrap().contains(needle));
            if found {
                seen += 1;
            }
        }
        let seg_found = seeds
            .iter()
            .any(|seed| seg_b_count(&generate(&data, &program, *seed).unwrap()) > 0);
        assert!(
            seen + usize::from(seg_found) >= 8,
            "{target}: only {seen} literal + {seg_found} templated new B-forms reached the shell"
        );
        // 抽查：模板 B 形态出现的次数必须与站点数一致（varint 一处、87 三处、
        // TU 一处、seg 每段六处），出现得更多说明模板误匹配。
        let raw = generate(&data, &program, seeds[0]).unwrap();
        assert_eq!(raw.matches("%128*128+g[").count(), 1, "{target}: varint B count");
        assert_eq!(raw.matches("*87+g[").count(), 3, "{target}: 87 B count");
        assert_eq!(raw.matches(".__obf_proto_nu+TU").count(), 1, "{target}: TU B count");
        assert!(
            seg_b_count(&raw) == 18,
            "{target}: seg B count {} != 18 (three segments x six accumulators)",
            seg_b_count(&raw)
        );
    }
}

/// K6 第二步：分组多态——装载点分 2..=4 组，每组一个折叠写法；跨种子时
/// (键 -> 写法) 的分配必须变化（固定分配意味着分组是装饰），同一种子必须
/// 逐字节可复现。
#[test]
fn k20_install_groups_are_seed_polymorphic() {
    let source = fs::read_to_string("tests/fixtures/vm_lua51.lua").unwrap();
    let data = compile(&source, Target::Lua51).unwrap();
    let program = custom::decode(&data, Target::Lua51).unwrap();
    let form_map = |seed: u64| -> std::collections::BTreeMap<u64, usize> {
        let raw = generate(&data, &program, seed).unwrap();
        let (installs, _) = scan_scatter(&raw);
        let mut per_key_sites: std::collections::BTreeMap<u64, usize> = std::collections::BTreeMap::new();
        for (key, _, _, _) in &installs {
            *per_key_sites.entry(*key).or_insert(0) += 1;
        }
        let mut map = std::collections::BTreeMap::new();
        for (key, _, _, form) in &installs {
            if per_key_sites[key] == 1 {
                map.insert(*key, *form);
            }
        }
        map
    ...
    let a = form_map(7001);
    let b = form_map(1);
    assert!(!a.is_empty() && !b.is_empty(), "no single-site installs to compare");
    assert_ne!(
        a, b,
        "the (key -> fold form) assignment is seed-independent: grouping is cosmetic"
    );
    // 复现性：同一种子两次生成，(键, 起点, 终点, 写法) 完全一致。
    let (i1, _) = scan_scatter(&generate(&data, &program, 7001).unwrap());
    let (i2, _) = scan_scatter(&generate(&data, &program, 7001).unwrap());
    assert_eq!(i1, i2, "install plan not reproducible for the same seed");
}
