...
fn m7_structural_variants_vary_across_seeds_and_stay_reproducible() {
    ...
    let forms = |output: &str, target: Target| -> (usize, usize, usize, usize, usize) {
        ...
        let mut call = 0usize;
        // L1: the `="userdata"` guard's string may be escape-spelled, so the
        // order probe must decode the operand's value instead of matching the
        // raw bytes.
        let mut userdata = [false, false];
        for index in 0..tokens.len() {
            if kind(index) != crate::lexer::TokenKind::String {
                continue;
            }
            if crate::minify::literal_bytes(text(index), target).unwrap() != b"userdata" {
                continue;
            }
            // `<expr> == "userdata"` / `~=` -- the comparison operator sits in
            // front of the string.
            if index > 0 && matches!(text(index - 1), "==" | "~=") {
                userdata[0] = true;
            }
            // `"userdata" == <expr>` / `~=` -- the operator follows the string.
            if index + 1 < tokens.len() && matches!(text(index + 1), "==" | "~=") {
                userdata[1] = true;
            }
        }
        for index in 0..tokens.len() {
    ...
    for (target, fixture) in [
        ...
        for seed in 0..=15u64 {
            ...
            assert_eq!(emit(&data, target, seed).unwrap(), output);
            let (dispatch, bound, call, ud0, ud1) = forms(&output, target);
            dispatch_total |= dispatch;
            bound_total |= bound;
            call_total |= call;
            userdata_seen[0] |= ud0 == 1;
            userdata_seen[1] |= ud1 == 1;
        }
    }
    assert!(
        dispatch_total >= 3,
...
fn core_logic_flows_through_scratch_table_slots() {
    ...
    for target in [Target::Lua51, Target::Luau] {
        ...
        for seed in 0..=11u64 {
            ...
            // K6 step 2 added the tenth table: the 87-checksum field is now
            // respelled at its three `*87` sites, so it qualifies for the
            // same-key double install and ships two equivalent slotted copies
            // (six `*87` sites total, one more scratch table + clear each).
