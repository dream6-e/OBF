    ...
    for target in [Target::Lua51, Target::Luau] {
        for seed in [0u64, 1, 3, 7001, 7351, 999983, u64::MAX] {
            ...
            for part in 0..3 {
                ...
                let lit = baked(&alpha);
                // L1: the table literal may be escape-spelled per fragment, so
                // the gate decodes every `..`-concatenated string run in the
                // finished script and compares the VALUES with the segment's
                // alphabet. Exactly one run is allowed to assemble to it.
                let count = {
                    let tokens = crate::lexer::lex(&output, target).unwrap();
                    let decode = |index: usize| {
                        crate::minify::literal_bytes(tokens[index].text(&output), target).unwrap()
                    };
                    let mut runs = 0usize;
                    for start in 0..tokens.len() {
                        if tokens[start].kind != crate::lexer::TokenKind::String {
                            continue;
                        }
                        let mut acc = decode(start);
                        if acc.len() > 11 {
                            continue;
                        }
                        ...
                        while acc.len() < 86
                            && cursor + 2 < tokens.len()
                            && tokens[cursor + 1].text(&output) == ".."
                            && tokens[cursor + 2].kind == crate::lexer::TokenKind::String
                        {
                            acc.extend(decode(cursor + 2));
                            cursor += 2;
                        }
                        if acc == alpha {
                            runs += 1;
                ...
                assert_eq!(
                    count, 1,
                    "{target} seed {seed}: segment {part}'s table appears {count} times",
                );
