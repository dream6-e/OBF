...
// Re-recorded per seed after K4 step 2 (the function-structure transform):
// the validator's arm union is the program's own ops (46 on Lua 5.1, 48 on
// Luau -- the fixture never executes `Export`, zero Export words in the
// bytecode) plus the ops the seeded semantic-image surface advertises (decoy
// recipes and camouflage descriptor advertisements, drawn from the seeded
// PRNG stream after the per-prototype plans) plus the two loader decoy arms.
// Before K4 the Luau union happened to carry two advertised extras at every
// seed (51). K4's deterministic prototype reorder moved the draw stream:
// seed 0's live descriptors no longer advertise `Export`, so its union is
// 50 while the other three seeds keep 51. The Lua 5.1 union is unchanged at
// 46+2 for all seeds. The tree's other properties (fail-closed leaves,
// seed-varying topology) are asserted below and still hold.
#[test]
fn validator_dispatch_is_a_seeded_binary_search_tree() {
    for (target, file, per_seed) in [
        (
            Target::Lua51,
            "tests/fixtures/vm_lua51.lua",
            &[(0u64, 48usize), (7001, 48), (7351, 48), (u64::MAX, 48)],
        ),
        (
            Target::Luau,
            "tests/fixtures/vm_luau.lua",
            &[(0u64, 50usize), (7001, 51), (7351, 51), (u64::MAX, 51)],
        ),
    ] {
        let source =
        ...
        for &(seed, arms_expected) in per_seed {
            let raw = generate(&data, &program, seed).unwrap();
...
#[test]
fn probe_dump_raw() {
    let src51 = fs::read_to_string("tests/fixtures/vm_lua51.lua").unwrap();
    let data51 = compile(&src51, Target::Lua51).unwrap();
    let prog51 = custom::decode(&data51, Target::Lua51).unwrap();
    let raw51 = generate(&data51, &prog51, 7001).unwrap();
    fs::write("/tmp/raw51.lua", &raw51).unwrap();
    let srcl = fs::read_to_string("tests/fixtures/vm_luau.lua").unwrap();
    let datal = compile(&srcl, Target::Luau).unwrap();
    let progl = custom::decode(&datal, Target::Luau).unwrap();
    let rawl = generate(&datal, &progl, 7351).unwrap();
    fs::write("/tmp/rawluau.lua", &rawl).unwrap();
    println!("dumped {} {}", raw51.len(), rawl.len());
}
