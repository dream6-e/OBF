...
///
/// K4/K6 re-record (2026-09-23), measured on the re-recorded golden, both rows moved
/// by K4's inlining and held byte-stable through the K6 stream redraw:
/// - `86` (2 -> 3): the third spelling is a ciphertext trigram inside a payload string,
///   not a code literal. The text census (this gate's and check1's rule) sees three; a
///   string-aware recount sees exactly one -- the shared base-86 decode radix site,
///   unchanged from the pre-K4 golden. The K6 redraw re-encoded the ciphertext but the
///   trigram count held 3 -> 3.
/// - `4294967295` (<= 2 -> 4): all four are code sites -- one wire-validator sentinel
///   (`x[23].u~=4294967295`) plus the stream hash's Mersenne modulus spelled three
///   times (`%4294967295`: one function definition and the two inlined copies K4's
///   transform left in the decode/cursor bodies). The K6 redraw held it 4 -> 4.
const SPELLING_MAXIMA_LUA51: [(u64, usize); 5] =
    [(86, 3), (256, 4), (4294967295, 4), (4294967296, 3), (2147483647, 24)];
