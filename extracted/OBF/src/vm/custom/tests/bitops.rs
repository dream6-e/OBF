...
fn k7_bit_library_names_are_never_spelled() {
    ...
    for (target, golden_path) in [
        ...
        // Data noise recorded 2026-09-23 with the K4/K6 golden re-record: the Luau
        // golden's base86 ciphertext spells the trigram `bor` exactly once (offset
        // 9303, wholly inside a payload ciphertext fragment -- every neighbouring
        // byte is an alphabet character, i.e. encrypted data, not a spelled name).
        // It is a stream-redraw collision (K4's inlining) that the K6 respellings
        // held at 1 -> 1. Only the shipped ciphertext earns its recorded count; the
        // probe output stays strict-zero for every word, and one further hit -- or
        // any other word, in either text -- is a real surface move.
        let golden_noise_cap = |word: &str| -> usize {
            if target.is_luau() && word == "bor" { 1 } else { 0 }
        };
        for (strict, text) in [(true, output.as_str()), (false, golden.as_str())] {
            for word in WORDS {
                let count = text.matches(word).count();
                if strict {
                    assert_eq!(count, 0, "{target}: {word} spelled in output");
                } else {
                    let cap = golden_noise_cap(word);
                    assert!(
                        count <= cap,
                        "{target}: {word} spelled {count}x in golden (recorded cap {cap})"
                    );
                }
            }
