...
impl Prng {
    ...
    pub fn dispatch_condition_for(&mut self, variable: &str, opcode: u16, luau: bool) -> String {
        ...
        self.dispatch_shape(variable, &literal, &[u64::from(opcode)], false)
    }
    ...
    pub fn dispatch_condition_opaque(
        ...
        // M2: one draw in four keys the comparison on an arithmetic rotate of
        // the variable instead of a reconstructed constant -- the bound's own
        // digits never appear beside it (rotated keys are rejected until they
        // do not), and folding the form needs multiplication-wide semantics on
        // the `*2^S % 2^32` lane.
        if bits && self.index(4) == 0 {
            return self.rol_dispatch_condition(variable, opcode, luau);
        }
        let literal = self.opaque_literal_scoped(u64::from(opcode), luau, bits, source);
        self.dispatch_shape(variable, &literal, &[u64::from(opcode)], bits)
    }
    ...
    fn dispatch_shape(
        ...
        if bits && self.index(3) == 0 {
            // M2 boolean wrappers; the operands must never equal the numbers
            // the condition is about (the bound, or this interval's noise),
            // or the census could silently count the anchor again.
            result = self.bool_wrap(&result, forbid);
        }
        result
    }
    ...
    pub fn boundary_condition(
        ...
        self.boundary_shape(variable, &literal, below, &[u64::from(bound)], false)
    }
    ...
    pub fn boundary_condition_opaque(
        ...
        let literal = self.opaque_literal_scoped(u64::from(bound), luau, bits, source);
        self.boundary_shape(variable, &literal, below, &[u64::from(bound)], bits)
    }
    ...
    pub fn opaque_literal(&mut self, value: u64, luau: bool, source: &str) -> String {
        self.opaque_literal_scoped(value, luau, false, source)
    }

    /// Same reconstruction, plus an opt-in for the M1 batch's Luau bit-composed
    /// families. The captured-low-level names (`BX/BA/BO/BN/LR/SHL/RS`) exist
    /// only in a handful of validator scopes (the loader chain destructures
    /// them as parameters); every caller therefore has to declare *explicitly*
    /// that the text it builds lands in such a scope, rather than the family
    /// leaking as nil-call text anywhere else. When `bits` is off, this is
    /// draw-for-draw identical to before this batch.
    pub(crate) fn opaque_literal_scoped(
        &mut self,
        value: u64,
        luau: bool,
        bits: bool,
        ...
        assert!(
            !bits || luau,
            "opaque_literal_scoped: bit-composed families need Luau"
        );
        ...
        if bits {
            arms.extend(self.opaque_bit_arms(value));
        }
        arms.retain(|arm| !Self::spells_value(value, arm));
        assert!(
            arms.len() >= 2,
        ...
        // M2: the two picked arms may ride through another exact-identity
        // family -- a self-cancelling zero-dwarf (either target) or, inside a
        // bit-captured scope, an all-ones `BA(arm, 0xFFFFFFFF)`. The two are
        // mutually exclusive per arm to keep the text short.
        let mut picked = [
            std::mem::take(&mut arms[first]),
            std::mem::take(&mut arms[second]),
        ];
        for slot in 0..picked.len() {
            // M2: both wrappers draw only inside bit-capture scopes, so the
            // R/D state-machine conditions -- whose text has dedicated
            // decoding gates -- keep their pre-M2 spellings draw-for-draw.
            if !bits {
                break;
            }
            let arm = std::mem::take(&mut picked[slot]);
            picked[slot] = match self.index(4) {
                0 => self.all_ones_bit_arm(arm, luau),
                1 => self.zero_dwarf_arm(arm, value),
                _ => arm,
            };
        }
        let guard = self.opaque_guard(source, value, luau);
        let text = format!("({guard}and{}or{})", picked[0], picked[1]);
        debug_assert!(
    ...
    pub(crate) fn audit_nice(value: u64) -> bool {
    ...
    fn opaque_guard(&mut self, source: &str, value: u64, luau: bool) -> String {
        ...
        // M2 note: `type(...)` guard forms were considered and **dropped** --
        // a bare `type` read is a plaintext global and the transport layer
        // hidden-name audit (K13) rejects it, while `TY` (its capture) is
        // threaded only through the semantic-buffer decode scope, never
        // through any opaque-predicate domain. Restoring the family needs
        // TY threaded through the validator parameter list first.
        if luau {
...
pub(crate) mod tests {
    ...
    impl<'a> OvParser<'a> {
        ...
        /// Lua equality at fold time: Numbers compare by value, booleans by
        /// truthiness, and the two kinds never equal each other.
        pub(crate) fn ov_equals(left: &OvValue, right: &OvValue) -> bool {
            match (left, right) {
                (OvValue::Num(a), OvValue::Num(b)) => a == b,
                (OvValue::Bool(a), OvValue::Bool(b)) => a == b,
                _ => false,
            }
        }

        pub(crate) fn parse(text: &str, source_value: i64) -> i64 {
        ...
        pub(crate) fn truthy(text: &str, source_value: i64) -> bool {
        ...
        fn compare_expr(&mut self) -> OvValue {
            ...
            // The M2 batch surfaces (`K-o>0` boundaries, `N~=0` boolean
            // wrappers) made `>`/`>=` and boolean-vs-boolean equality real
            // inputs, so both join the comparator set with exact Lua
            // semantics: a boolean equals only another boolean with the same
            // truthiness, and never equals any number.
            if self.eat("<=") {
            ...
            if self.eat("==") {
                ...
                return OvValue::Bool(Self::ov_equals(&left, &right));
            }
            if self.eat("~=") {
                let right = self.sum_expr();
                return OvValue::Bool(!Self::ov_equals(&left, &right));
        ...
        fn atom(&mut self) -> OvValue {
            // M2: `type(EXPR) op "TAG"` -- every opaque condition variable is a
            // numeric lane at every site, so the tag is decided statically.
            if self.peek() == Some(b't') && self.eat("type") {
                assert!(self.eat("("), "type( must follow");
                let _inner = self.or_expr();
                assert!(self.eat(")"), "type() must close");
                let equal = self.eat("==");
                if !equal {
                    assert!(self.eat("~="), "type() needs a comparison");
                }
                assert_eq!(self.peek(), Some(b'"'), "type() needs a tag string");
                self.at += 1;
                let tag_start = self.at;
                while self.peek() != Some(b'"') {
                    self.at += 1;
                }
                let tag = &self.bytes[tag_start..self.at];
                self.at += 1;
                return OvValue::Bool(equal == (tag == b"number"));
            }
            // M1: the Luau-only hidden-name bit calls (`BX(A,B)` etc. are
            // destructured validator parameters bound to bit32 functions).
            // Fold with u32-exact semantics, exactly as the captures behave.
            let upper_end = {
                let mut cursor = self.at;
                while cursor < self.bytes.len() && self.bytes[cursor].is_ascii_uppercase() {
                    cursor += 1;
                }
                cursor
            };
            if upper_end > self.at + 1
                && upper_end < self.bytes.len()
                && self.bytes[upper_end] == b'('
            {
                let name: Vec<u8> = self.bytes[self.at..upper_end].to_vec();
                self.at = upper_end;
                assert!(self.eat("("), "hidden-name call {:?} misses (", name);
                let mut args: Vec<i64> = Vec::new();
                loop {
                    args.push(self.or_expr().num());
                    if self.eat(",") {
                        continue;
                    }
                    assert!(self.eat(")"), "hidden-name call {:?} misses )", name);
                ...
                let lane = |value: i64| value as u64 as u32;
                let folded: u32 = match name.as_slice() {
                    b"BN" => {
                        assert!(args.len() == 1);
                        !lane(args[0])
                    }
                    b"BX" | b"BA" | b"BO" | b"LR" | b"SHL" | b"SHR" => {
                        assert_eq!(args.len(), 2, "hidden-name call {:?} arity", name);
                        match name.as_slice() {
                            b"BX" => lane(args[0]) ^ lane(args[1]),
                            b"BA" => lane(args[0]) & lane(args[1]),
                            b"BO" => lane(args[0]) | lane(args[1]),
                            b"LR" => lane(args[0]).rotate_left((args[1] as u32) % 32),
                            b"SHL" => lane(args[0]) << ((args[1] as u32) % 32),
                            _ => lane(args[0]) >> ((args[1] as u32) % 32),
                        }
                    }
                    _ => panic!("unknown hidden name {:?}", name),
                };
                return OvValue::Num(i64::from(folded));
            }
            if self.eat("(") {
    ...
    pub(crate) fn opaque_parts<'a>(text: &'a str) -> (&'a str, &'a str, &'a str) {
        let body = text
            .strip_prefix('(')
    ...
    fn opaque_conditions_never_spell_their_bound() {
        ...
        for luau in [false, true] {
            for bound in [17u16, 86, 256, 512, 4096, 12_345, 65_535] {
                for _ in 0..8 {
                    let text = random.dispatch_condition_opaque("o", bound, luau, false, "o");
                    assert!(!Prng::spells_value(u64::from(bound), &text), "{text}");
                    let text = random.boundary_condition_opaque("o", bound, true, luau, false, "o");
                    assert!(!Prng::spells_value(u64::from(bound), &text), "{text}");
                    let text =
                        random.boundary_condition_opaque("o", bound, false, luau, false, "o");
                    assert!(!Prng::spells_value(u64::from(bound), &text), "{text}");
                    let text =
                        random.interval_condition_opaque("o", bound, 7, true, luau, false, "o");
                    assert!(!Prng::spells_value(u64::from(bound), &text), "{text}");
                    let text =
                        random.interval_condition_opaque("o", bound, 7, false, luau, false, "o");
                    assert!(!Prng::spells_value(u64::from(bound), &text), "{text}");
                }
            }
        }
