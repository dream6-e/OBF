...
fn run() -> Result<(), Diagnostic> {
    ...
    // One-step obfuscator mode: `obf input.lua --luau|--lua51 [--MB] ...` --
    // the positional first argument IS the source file, and the command word
    // collapses to the internal `obfuscate` route. The classic subcommands
    // above keep their forms unchanged.
    let known_commands = [
        "check",
        "minify",
        "virtualize",
    ...
    let obfuscate_mode = !known_commands.contains(&command.as_str());
    let obfuscate_input = if obfuscate_mode {
        let path = std::path::Path::new(&command);
        let looks_like_input =
            path.is_file() || command == "-" || path.extension().is_some_and(|ext| ext == "lua");
        if !looks_like_input {
            return Err(Diagnostic::new(format!(
                "unknown command '{command}' (for the one-step obfuscator mode, pass a .lua input first and add --luau|--lua51 [--MB])"
            )));
        }
    ...
    let command = if obfuscate_mode {
        String::from("obfuscate")
    } else {
        command
    };
    ...
    while let Some(argument) = rest.next() {
        match argument.as_str() {
            "-t" | "--target" => {
                ...
                if obfuscate_mode {
                    target_flags_seen = true;
                }
            }
            // POSIX end-of-options marker: cargo habits make users type
            // `obf input.lua -- lua51`; the one-step mode simply skips it.
            // Classic subcommands keep the strict parser surface and still
            // reject it as an unknown option.
            "--" => {
                if !obfuscate_mode {
                    return Err(Diagnostic::new("unknown option '--'"));
                }
            }
            // One-step mode sugar: `--luau` / `--lua51` select the backend
            // without a value, `--MB` asks for the compressed self-expanding
            // shell (`off` by default). Classic commands still reject them,
            // keeping the dual-surface error contract tight.
            "--luau" | "--lua51" => {
                if !obfuscate_mode {
                    return Err(Diagnostic::new(format!(
                        "{argument} is only valid in the one-step obfuscator mode (obf INPUT.lua {argument})"
                    )));
                }
                if target_flags_seen {
                    return Err(Diagnostic::new(
                        "--target cannot be combined with --luau/--lua51",
                    ));
                }
                if target.is_some() {
                    return Err(Diagnostic::new("choose exactly one of --luau or --lua51"));
                }
                target = Some(parse_target(&argument[2..])?);
            }
            // The same two backends are accepted as bare words
            // (`obf input.lua lua51`), exactly as the original ask spelled
            // them, with identical exclusivity rules as the dashed sugar.
            _ if obfuscate_mode && matches!(argument.as_str(), "luau" | "lua51") => {
                if target_flags_seen {
                    return Err(Diagnostic::new(
                        "--target cannot be combined with --luau/--lua51",
                    ));
                ...
                target = Some(parse_target(&argument)?);
            }
            "--MB" => {
                if !obfuscate_mode {
                    return Err(Diagnostic::new(
                        "--MB is only valid in the one-step obfuscator mode (obf INPUT.lua --luau --MB)",
                    ));
                }
                compressed = true;
            }
    ...
    let target = target.ok_or_else(|| {
        if obfuscate_mode {
            Diagnostic::new(
                "choose the backend for the one-step obfuscator mode: --luau or --lua51",
            )
        } else {
            Diagnostic::new("--target is required")
        }
    })?;
    let input =
    ...
    if seed_was_set
        && !matches!(
            command.as_str(),
            "minify" | "virtualize" | "wrap-bytecode" | "shell" | "obfuscate"
        )
    {
        return Err(Diagnostic::new(
            "--seed is only valid for 'minify', 'virtualize', 'wrap-bytecode', 'shell' or the one-step obfuscator mode",
        ));
    }
    ...
    // One-step mode hands users a file: without `-o` the obfuscated script
    // lands in `obfuscated.lua` in the current directory instead of streaming
    // to stdout. Classic subcommands keep their stdout default; `-o` overrides.
    if obfuscate_mode && output.is_none() {
        eprintln!("output: obfuscated.lua (default name; pass -o FILE to override)");
        output = Some(PathBuf::from("obfuscated.lua"));
    }
    ...
    match command.as_str() {
        ...
        "virtualize" | "obfuscate" => {
            ...
            if obfuscate_mode && compressed {
                let shell = obf::shell::wrap(&result, target, seed)?;
                eprintln!(
                    "obfuscate: target={} source={}B vm={}B compressed={}B base85={}B tokens={} ratio={:.3}",
                    target,
                    data.len(),
                    result.len(),
                ...
                return write_output(output, shell.script.as_bytes());
            }
            if obfuscate_mode {
                eprintln!(
                    "obfuscate: target={} source={}B vm={}B (add --MB for the compressed shell)",
                    target,
                    data.len(),
                    result.len()
                );
...
Usage:\n  obf INPUT.lua --luau|--lua51 [--MB] [-o FILE] [--seed N]  (one-step obfuscator mode)\n  \
...
One-step obfuscator mode: the first argument is the .lua input (or `-` for stdin);\n\
then pass the backend once as --luau/--lua51 (bare `luau`/`lua51` work too) and\n\
optionally --MB for the compressed self-expanding shell (off by default). A lone\n\
`--` between the input and the flags is tolerated. The obfuscated script is\n\
written to ./obfuscated.lua by default; pass -o FILE to choose another name.\n\
