...
Usage: python obfuscators/luraph_v14_9/vmmap.py <protected.luau> [dispatch_index] [opcode...]
"""
import json
import os
import subprocess
...
def _select_vm_factory(stack, stmts, disp):
    ...
    Across the supplied 14.7/14.9/14.9 layouts the stable property is not an
...
def maker_info(root):
    """Return runtime-capture metadata for v14.7, v14.9 and v14.9 makers."""
    dispatches = find_dispatchers(root)
    by_node = {id(d["node"]): d for d in dispatches}
    out = {}
...
def _outer_captures(info):
    ...
    v15 makers are usually self-contained.  v14.7/v14.9 can install the VM
