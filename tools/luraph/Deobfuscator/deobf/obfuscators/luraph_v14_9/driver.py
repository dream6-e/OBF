"""
Luraph v14.9 pipeline (see LURAPH.md).
...
def patch_entries(source, path):
    ...
    from obfuscators.luraph_v14_9 import vmmap
...
def devirtualize(job, ppath, dpath, cfg, rerun, chunk_paths=(), live=None):
    ...
    from obfuscators.luraph_v14_9 import devirt
    args = job.args
    requested = set()
    last_bufs = ""
    text = None
...
def run(job):
    ...
    trapped_kind = None # "anti" or "alloc"
    cfg = None
    lenient_alloc = False
    for attempt in range(1, args.max_runs + 1):
        ...
        if devirt_on:
            cfg["devirt"] = True
        # v14.9 has one Roblox fingerprint path that can derive an invalid
        # table.create/buffer.create size in the standalone harness.  Attribute
        # the failing allocation to its active proto for diagnostics; do not
        # skip it or fabricate replacement allocations.
        if not os.environ.get("DEOB_V14_NO_ALLOC_TRAP"):
            cfg["v14_alloc_trap"] = True
        if lenient_alloc:
        ...
        if added:
            ...
            continue
        alloc_trig = re.search(r"\x00ALLOCTRIGGER (\d+)(?: ([^ \n]+))?(?: ([^\n]+))?", body)
        trig = re.search(r"\x00TRIGGER (\d+)", body)
        active_trig = alloc_trig or trig

        if (trapped is not None and trapped_kind != "alloc"
                and trace.stmt_count(body) < trace.stmt_count(trapped[0])):
            # For ordinary anti-tamper traps, a shorter rerun usually means we
            # disabled the script's own crash/check function. Allocation-probe
            # helpers are different: shortening the rerun is expected, so never
            # roll those back merely because fewer probe statements executed.
        ...
        if not active_trig:
            # Keep the same behavior as the working v14.8 driver.  The only
            # v14.9 special case is allocation *attribution* above; lenient
            # replacement allocations remain opt-in for diagnostics.
            if (os.environ.get("DEOB_V14_LENIENT_ALLOC")
                ...
                print("[*] v14.9 allocation fingerprint was not attributable; re-running once with lenient probe allocations",
                      file=sys.stderr)
                lenient_alloc = True
                continue
            if (not lenient_alloc and "size out of range" in body and "script error:" in body):
                print("[!] v14.9 allocation fingerprint was not attributable; keeping the first trace",
                      file=sys.stderr)
            break

        trapped = (body, harness.LAST_RAW[0])
        trapped_kind = "alloc" if alloc_trig else "anti"
        pid = int(active_trig.group(1))
        if pid in skip:
            what = "allocation fingerprint" if alloc_trig else "anti-tamper trigger"
            print("[!] %s helper #%d fired again; giving up on reruns" % (what, pid), file=sys.stderr)
            break

        if alloc_trig:
            # Allocation errors identify the active VM, not necessarily a
            # disposable fingerprint helper. Skipping this proto can skip the
            # entire bootstrap/decoder and replace the useful error with nil.
            kind = alloc_trig.group(2) or "create"
            size = (alloc_trig.group(3) or "?").strip()
            print("[!] invalid %s.create(%s) in proto #%d: keeping the original capture; "
                  "refusing to skip an unproven fingerprint helper" % (kind, size, pid), file=sys.stderr)
            break
    ...
    if not devirt_on:
        runner.finish()
    harness.save_raw(args.raw)
    body = re.sub(r"\x00ALLOCTRIGGER \d+(?: [^ \n]+)?(?: [^\n]+)?\n?", "", body)
